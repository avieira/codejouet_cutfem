...
void break_edges_split_fusion(const Polygon2D* old_p, const Vector_uint* edge_intersect1, const Vector_uint* edge_intersect2, \
    ...
    for(i=0; i<p->vertices->size; i++){
        if(pt_in_or_out[i]<0){
                ...
                if(pt_in_or_out[j]<0){
                    ...
                    if (listEdges->size>0){
                        ...
                        for(l=0; l<nb_faces; l++){
                            for(k=0; k<listEdges->size; k++){
                                ind = *get_ith_elem_vec_uint(listEdges, k);
                                GrB_Matrix_removeElement(*(p->faces), ind, l);
                            }
                        }
            ...
            //GrB_assign(*(p->edges), GrB_NULL, GrB_NULL, 0, (GrB_Index[]){i}, 1, GrB_ALL, 1, GrB_NULL);
...
void polygons_fusion(const Polygon2D* old_p, const GrB_Matrix* original_edges, const Vector_points2D* pts_intersec, const Vector_uint* edge_intersect1, const Vector_uint* edge_intersect2, const int8_t* pt_in_or_out,\
    ...
    GrB_Matrix f_k, new_pt, new_edges, new_faces;
    int8_t sum_col1, sum_col2, orient_edge, val;
    GrB_Index nvals, nb_edges, nb_pts, orig_pts1, orig_pts2, size_nz_ej;
    GrB_Index I_ind[2];
    Point2D *pt1, *pt2, *pt;
    ...
    *faces_to_fuse = (Vector_uint**)malloc(former_nb_faces*sizeof(Vector_uint*));
    ...
    for (i=0; i<edge_intersect1->size; i++){
        ...
        if (sum_col1!=0){
            GrB_Matrix_setElement(new_pt, -sum_col1/abs(sum_col1), 1, ie1);
            GrB_Matrix_setElement(new_pt, sum_col1/abs(sum_col1), 1, ie2);
        } else if (sum_col2!=0) {
            GrB_Matrix_setElement(new_pt, -sum_col2/abs(sum_col2), 1, ie2);
            GrB_Matrix_setElement(new_pt, sum_col2/abs(sum_col2), 1, ie1);
        } else { //edges ie1 and ie2 intersect but none are broken: what should we do?
            GrB_Matrix_setElement(new_pt, 1, 1, ie1);
            GrB_Matrix_setElement(new_pt, 1, 1, ie2);
        }
        GxB_Matrix_concat(*((*p)->edges), (GrB_Matrix[]){*((*p)->edges), new_pt}, 2, 1, GrB_DESC_R);
    }
    ...
    for(i=0; i<nb_edges; i++){
        ...
        if (sum_col1>2){
            ...
            GrB_extract(f_k, GrB_NULL, GrB_NULL, *((*p)->faces), &i, 1, GrB_ALL, 1, GrB_NULL); 
            GrB_Matrix_nvals(&nvals, f_k);
            GrB_Vector_resize(pot_ind_faces, nvals);
            GrB_Vector_resize(I_vec_f_k, nvals);
            GrB_Vector_resize(extr_vals_f_k, nvals);
            ...
            GrB_Matrix_extractElement(&orient_edge, *((*p)->faces), i, i_face1);

            //ej = p.edges[0:former_nb_pts-1, i]
            GrB_extract(ej, GrB_NULL, GrB_NULL, *(original_edges), GrB_ALL, 1, i, GrB_NULL); 
            GxB_Vector_extractTuples_Vector(nz_ej, extr_vals_ej, ej, GrB_NULL);
            GrB_Vector_extractElement(&orig_pts1, nz_ej, 0);
            GrB_Vector_extractElement(&orig_pts2, nz_ej, 1);

            GrB_Matrix_extractElement(&val, *(original_edges), orig_pts1, i);
            ...
            GrB_extract(ej, GrB_NULL, GrB_NULL, *((*p)->edges), I_ind, GxB_RANGE, i, GrB_NULL); 
            GxB_Vector_extractTuples_Vector(nz_ej, extr_vals_ej, ej, GrB_NULL);
            GrB_Vector_size(&size_nz_ej, nz_ej);
            thetas->size = 0;
            for(j=0; j<size_nz_ej; j++){
                GrB_Vector_extractElement(&k, nz_ej, j);
                k += former_nb_pts;
            ...
            GrB_Matrix_resize(new_edges, nb_pts, indices_sorted->size/2);
            GrB_Matrix_clear(new_edges);
            for(j=0; j<nb_pts; j++){
                GrB_Matrix_removeElement(*((*p)->edges), j, i);
            }
            if (pt_in_or_out[i_pt1]>0){
                //p.edges[i_pt1, i] = -1
                GrB_Matrix_setElement(*((*p)->edges), -1, i_pt1, i);
                //p.edges[nz_ej[indices_sorted[1]] + former_nb_pts, i] = 1
                GrB_Vector_extractElement(&k, nz_ej, *get_ith_elem_vec_uint(indices_sorted, 0));
                k += former_nb_pts;
                GrB_Matrix_setElement(*((*p)->edges), 1, k, i);
            
                for (j = 1; j<indices_sorted->size-1 ; j += 2){
                    index_new_edge = j/2;
                    //new_edges[nz_ej[indices_sorted[j]],   index_new_edge] = -1
                    GrB_Vector_extractElement(&k, nz_ej, *get_ith_elem_vec_uint(indices_sorted, j));
                    k += former_nb_pts;
                    GrB_Matrix_setElement(new_edges, -1, k, index_new_edge);
                    //new_edges[nz_ej[indices_sorted[j+1]], index_new_edge] =  1
                    GrB_Vector_extractElement(&k, nz_ej, *get_ith_elem_vec_uint(indices_sorted, j+1));
                    k += former_nb_pts;
                    GrB_Matrix_setElement(new_edges, 1, k, index_new_edge);
                }
            } else {
            ...
            if (pt_in_or_out[i_pt2]>0){
                index_new_edge = indices_sorted->size/2;
                //new_edges[nz_ej[indices_sorted[end]], end] = -1
                GrB_Vector_extractElement(&k, nz_ej, *get_ith_elem_vec_uint(indices_sorted, indices_sorted->size-1));
                k += former_nb_pts;
                GrB_Matrix_setElement(new_edges, -1, k, index_new_edge);
                //new_edges[i_pt2, end] = 1
                GrB_Matrix_setElement(new_edges, 1, i_pt2, index_new_edge);
            }

            //p.edges = [p.edges new_edges]
            GxB_Matrix_concat(*((*p)->edges), (GrB_Matrix[]){*((*p)->edges), new_edges}, 1, 2, GrB_DESC_R);
            ...
            GrB_Matrix_resize(new_faces, indices_sorted->size/2, former_nb_faces);
            GrB_Matrix_clear(new_faces);
            GrB_assign(new_faces, GrB_NULL, GrB_NULL, orient_edge, GrB_ALL, 1, &i_face1, 1, GrB_NULL);
            //p.faces = [p.faces; new_faces]
            GxB_Matrix_concat(*((*p)->faces), (GrB_Matrix[]){*((*p)->faces), new_faces}, 2, 1, GrB_DESC_R);
        }
    }
        ...
        if (s->size != 0){ //face i is split
            for(j=0; j<s->size; j++){
                ...
                if (sum_col != 0){
                    ...
                    GrB_extract(old_face, GrB_NULL, GrB_NULL, *(p->faces), listEdges->data, listEdges->size, i, GrB_NULL); 
                    GrB_assign(new_face, GrB_NULL, GrB_NULL, old_face, listEdges->data, listEdges->size, GrB_NULL);
                    //p.faces[listEdges, i] .= 0
                    for(k=0; k<listEdges->size; k++){
                        GrB_Matrix_removeElement(*(p->faces), *get_ith_elem_vec_uint(listEdges, k), i);
                    }
                    GrB_apply(old_face, GrB_NULL, GrB_NULL, GrB_ABS_INT8, new_face, GrB_NULL); //abs on each component
                    GrB_reduce(&sum_col, GrB_NULL, GrB_PLUS_MONOID_INT64, old_face, GrB_NULL);
                    if (sum_col != 0){
                        //p.faces = [p.faces new_face]
                        GrB_Matrix_clear(concat_face);
                        GrB_Col_assign(concat_face, GrB_NULL, GrB_NULL, new_face, listEdges->data, listEdges->size, 0, GrB_NULL);
                        GxB_Matrix_concat(*(p->faces), (GrB_Matrix[]){*(p->faces), concat_face}, 1, 2, GrB_DESC_R);
...
void polygon_split(const Polygon2D* old_p, const GrB_Matrix* original_edges, const Vector_points2D* pts_intersec, const Vector_uint* edge_intersect1, const Vector_uint* edge_intersect2, const int8_t* pt_in_or_out,\
    ...
    *faces_to_split = (Vector_uint**)malloc(former_nb_faces*sizeof(Vector_uint*));
    ...
    for(i=0; i<edge_intersect1->size; i++){
        ...
        GrB_Vector_resize(extr_vals_f_k, nvals);
        GxB_Matrix_extractTuples_Vector(I_vec_f_k, pot_ind_faces, extr_vals_f_k, f_k, GrB_NULL);
        GrB_Vector_extractElement(&i_face, pot_ind_faces, 0);
    ...
    for(i=0; i<nb_edges; i++){
        ...
        if (sum_col1>2){
            ...
            GrB_Matrix_extractElement(&orient_edge, *((*p)->faces), i, i_face);
            ...
            GrB_Matrix_resize(new_faces, indices_sorted->size/2, former_nb_faces);
            GrB_Matrix_clear(new_faces);
            GrB_assign(new_faces, GrB_NULL, GrB_NULL, orient_edge, GrB_ALL, 1, &i_face, 1, GrB_NULL);
            //p.faces = [p.faces; new_faces]
            GxB_Matrix_concat(*((*p)->faces), (GrB_Matrix[]){*((*p)->faces), new_faces}, 2, 1, GrB_DESC_R);
        }
    }
